...
class XooUserPhoto {
	...
	public function ini_photo_module_pro()
	{
		add_action( 'init', array( $this, 'ini_media_modules_pro' ));	
		
	}
	
	public function ini_media_modules_pro()
	{
		//----------------------------------------------
		//----------register and label gallery post type
		//----------------------------------------------
		$gallery_labels = array(
			'name' => _x('Ultra Gallery', 'post type general name'),
		...
		register_taxonomy( 'phototype', 'gallery',
				array(
					'hierarchical'=> true,
					'label' => 'Photo Types',
			...
			$this->uultra_add_categories_to_attachments();	
	...
	function uultra_add_categories_to_attachments() 
	{
	...
	function update_photos_table()
	{
		...
		//remove soon, created date 08/21/2014
		
		$sql = 'Alter table  ' . $wpdb->prefix . 'usersultra_videos modify  column video_unique_vid text ; ';
		$wpdb->query($sql);
	...
	public function add_new_gallery ()
	{
		...
		if (is_user_logged_in() && isset($user_id) && isset($gall_name)) 
		{
			...
			{
				$new = array(
				  'post_title'    =>$gall_name,
				  'post_type'     => 'gallery',
				  'post_name'     => $gall_name,			 
				  'post_content'  => $gall_description,
				  'post_status'   => 'publish',
			...
			}else{	//uses the old media method
	...
	public function get_user_photos($user_id,  $howmany)
	{
		...
		/*Latest Photos Profile*/
	public function get_latest_photos_public($user_id, $howmany)
	{
		global $wpdb, $xoouserultra;
		...
		$sql .= " WHERE gal.gallery_id = photo.photo_gal_id AND u.ID = gal.gallery_user_id AND u.ID= ".$user_id." ORDER BY photo.photo_id DESC  LIMIT $howmany";	
			...
			if (get_option('uultra_new_media_feature_pro') ==1)  // use the new media method 06-10-2014
			{
				...
				if( $author_posts->have_posts() ) 
				{
					while( $author_posts->have_posts() )
					{
						...
						$gallery_id = $author_posts->post->ID;	
						$gallery_name = $author_posts->post->post_title;
						$gallery_content = $author_posts->post->post_content;
						
						//get main picture
						$thumb = $this->get_main_picture($gallery_id);
						
						//get amount of pictures
						$amount_pictures = $this->get_total_pictures_of_gal($gallery_id);					
						
						echo "<li class='xoousersultra-shadow-borers' id='".$gallery_id."'>
						
						<div class='pe_icons_gal'>
						<a href='#resp_del_gallery' data-id='".$gallery_id."' class='delete' id='".$gallery_id."' alt='delete' title='".__( 'delete', 'xoousers' )."'></a>
						
						<a href='#resp_edit_gallery' data-id='".$gallery_id."' class='edit' id='".$gallery_id."' alt='edit' title='".__( 'edit', 'xoousers' )."'></a>
						</div>	
						
						<div class='usersultra-photo-name'>
						
							<a href='".$xoouserultra->userpanel->get_internal_links('photos-files','gal_id',$gallery_id)."' >".$gallery_name." </a>
						
						</div>
					
						<a href='".$xoouserultra->userpanel->get_internal_links('photos-files','gal_id',$gallery_id)."' class=''  ><img src='".$thumb."' /> </a>					
						
						<p class='usersultra-amount_pictures'>".$amount_pictures." ".__( 'Picture(s)', 'xoousers' )."</p>
						
						<p>".$gallery_content."</p>
						
						<div class='uultra-gallery-edit' id='gallery-edit-div-".$gallery_id."'>
						</div>
						</li>";	
		...
		if (isset($user_id)) 
		{
			...
			if ( empty( $videos ) )
			{
				...
				{								
					...
					<div class='pe_icons_gal'>
					<a href='#resp_del_gallery' data-id='".$video->video_id."' class='".__( 'delete', 'xoousers' )."' id='".$video->video_id."' alt='delete' title='".__( 'delete', 'xoousers' )."'></a>
					</div>";
					...
					switch($video->video_type): case "youtube":
					 ...
					 case "embed": 
							...
							$html .= stripslashes($video->video_unique_vid);
							$html .= "<p class='social_v'><i class='fa fa-play-circle-o  fa-3x'></i></p> ";
	...
	/*Get Videos Public*/
	public function get_videos_public_widget ($user_id, $howmany)
	{
		global $wpdb, $xoouserultra;
		...
		if (isset($user_id)) 
		{
			$rows = $wpdb->get_results( 'SELECT * FROM ' . $wpdb->prefix . 'usersultra_videos WHERE `video_user_id` = "' . $user_id . '" ORDER BY `video_order` ASC  LIMIT '.$howmany.' ' );
	...
	public function get_latest_gallery_public ($user_id, $howmany)
	{
		global $wpdb , $xoouserultra;
		...
		if (isset($user_id)) 
		{
			$rows = $wpdb->get_results( 'SELECT `gallery_id`, `gallery_user_id`, `gallery_name`, `gallery_desc`  FROM ' . $wpdb->prefix . 'usersultra_galleries WHERE `gallery_user_id` = "' . $user_id . '" ORDER BY `gallery_order` ASC LIMIT '.$howmany.' ' );	
		...
		if (isset($user_id)) 
		{
			...
			if ( empty( $galleries ) )
			{
				...
				foreach ( $galleries as $gall )
				{
					...
					$html.= '<li class="xoousersultra-shadow-borers" id="'.$gall->gallery_id.'">';
					
										
					$html.= ' <div class="usersultra-photo-name"> ';
					
					$html .= "<a href='". $xoouserultra->userpanel->public_profile_get_album_link($gall->gallery_id, $user_id)."' >".$gall->gallery_name." </a>
	...
	public function get_total_videos($user_id)
	{
		global $wpdb, $xoouserultra;
		
		$total = 0;

		$photos = $wpdb->get_results( 'SELECT count(*) as total  FROM ' . $wpdb->prefix . 'usersultra_videos WHERE `video_user_id` = "' . $user_id . '" ' );
	...
	public function get_total_photos ($user_id)
	{
		global $wpdb, $xoouserultra;
		
		$sql = 'SELECT  count(*) as total, photo.*, gal.gallery_user_id , gal.gallery_id , gal.gallery_private  
		               
					   FROM ' . $wpdb->prefix . 'usersultra_photos photo
					   
					    RIGHT JOIN ' . $wpdb->prefix . 'usersultra_galleries gal ON (gal.gallery_id = photo.photo_gal_id )
						
					   WHERE gal.gallery_user_id = ' . $user_id . ' AND gal.gallery_id = photo.photo_gal_id  ';
			...
			$args = array(
				...
				if( $posts->have_posts() ) 
				{
					...
					{						
						...
						$user_id = $posts->post->post_author;					
						
						//print_r($posts);
						$photo =  get_post_meta( $post_id, 'photo_thumb' , true);										
						$thumb = $site_url.$upload_folder."/".$user_id."/".$photo;
  ...
  <option value='1' ".$registered.">".__( 'Only Registered', 'xoousers' )."</option>
  <option value='2' ".$friends."> ".__( 'Only Friends', 'xoousers' )."</option>
	...
	public function edit_video_confirm ()
	{
		...
		$video_name = sanitize_text_field($_POST["video_name"]);
		$video_unique_id = $_POST["video_unique_id"];
		$video_type = sanitize_text_field($_POST["video_type"]);
	...
	public function edit_video ()
	{
		...
		if($video_id!="")
		{
			...
			foreach ( $res as $gal )
			{
				...
				if($gal->video_type=='embed'){$embed = "selected='selected'";}
				
				$html .="<p>".__( 'Name', 'xoousers' )."</p>";
				
				$html .="<p><input type='text' value='".$gal->video_name."' class='xoouserultra-input' id='uultra_video_name_edit_".$gal->video_id."'></p>";
				
				$html .="<p>".__( 'Video ID / URL', 'xoousers' )."</p>";				
				$html .="<p><input type='text' value='".stripslashes($gal->video_unique_vid)."' class='xoouserultra-input' id='uultra_video_id_edit_".$gal->video_id."'></p>";
  ...
  <option value='vimeo' ".$vimeo.">Vimeo</option>
   <option value='embed' ".$embed.">Embed</option>
	...
	public function delete_photo ()
	{
		...
		//delete from activity wall		
		$query = "DELETE FROM " . $wpdb->prefix ."usersultra_wall WHERE  `comment_module` = 'photo' AND comment_module_item_id = '$photo_id' ";						
		$wpdb->query( $query );	
	...
	public function get_photos_of_gal_public ($gal_id, $display_photo_rating, $gallery_type)
	{
		global $wpdb, $xoouserultra;
			...
			if (get_option('uultra_new_media_feature_pro') ==1)  // use the new media method 06-10-2014
			{
				...
				if( $author_posts->have_posts() ) 
				{
					while( $author_posts->have_posts() )
					{
						...
						$author_posts->the_post();						
						$photos_id = $author_posts->post->ID;	
						$photo_name = $author_posts->post->post_title;
						$gallery_content = $author_posts->post->post_content;						
						
						//get thumbnail						
						$thumb = $site_url.$upload_folder."/".$user_id."/".$photo_thumb;						
						$large = $site_url.$upload_folder."/".$user_id."/".$photo_large;
				...
				if ( empty( $photos ) )
				{
					...
					foreach ( $photos as $photo )
					{
						...
						echo "<li id='".$photo->photo_id."' >
						<div class='pe_icons'><a href='#resp_del_photo' data-id='".$photo->photo_gal_id."' class='delete' id='".$photo->photo_id."' alt='".__( 'delete', 'xoousers' )."'></a><a href='#resp_set_main' data-id='".$photo->photo_gal_id."' class='' id='".$photo->photo_id."'></a>
				   ...
				   if(!is_dir($path_pics."/".$o_id."")) 
				   {
					...
					if (copy($file['tmp_name'], $pathBig)) 
					{
						...
						if (get_option('uultra_new_media_feature_pro') ==1)  // use the new media method 06-10-2014
						{
							...
							update_post_meta($new_page, 'photo_large', $pic1);
							update_post_meta($new_page, 'photo_thumb', $pic2);
							update_post_meta($new_page, 'photo_mini', $pic3);
							//update_post_meta($new_page, 'photo_order', $order);
							update_post_meta($new_page, 'photo_main', '0');
						...
						$xoouserultra->wall->wall_save_activity($item_id, 'photo')	;
