...
class XooOrder 
{
	...
	public function ini_db()
	{
		...
		$query = 'CREATE TABLE IF NOT EXISTS ' . $wpdb->prefix . 'usersultra_orders (
				...
				`order_expiration` date NOT NULL,					 			
	...
	function update_table()
	{
		...
		$sql ='SHOW columns from ' . $wpdb->prefix . 'usersultra_orders where field="order_expiration" ';		
		$rows = $wpdb->get_results($sql);		
		if ( empty( $rows ) )
		{	
			$sql = 'Alter table  ' . $wpdb->prefix . 'usersultra_orders add column order_expiration date NOT NULL ; ';
			$wpdb->query($sql);
		}
		
		$sql ='SHOW columns from ' . $wpdb->prefix . 'usersultra_orders where field="order_type" ';		
		$rows = $wpdb->get_results($sql);		
		if ( empty( $rows ) )
		{	
			$sql = 'Alter table  ' . $wpdb->prefix . 'usersultra_orders add column order_type int(1) NOT NULL DEFAULT "1"; ';
			$wpdb->query($sql);
		}
	...
	public function create_order ($orderdata)
	{
		...
		//update database
		$query = "INSERT INTO " . $wpdb->prefix ."usersultra_orders (`order_user_id`,`order_package_id`, `order_method_name`, `order_key` ,`order_status` ,`order_amount` ,`order_type` , `order_date`) VALUES ('$user_id','$order_package_id','$method','$transaction_key', '$status','$amount', '$type', '".date('Y-m-d')."')";
	...
	public function update_expiration_date ($id,$expiration_date)
	{
		...
		//update database
		$query = "UPDATE " . $wpdb->prefix ."usersultra_orders SET order_expiration = '$expiration_date' WHERE order_id = '$id' ";
		$wpdb->query( $query );
